...
library(ggplot2)
#### Source Functions ####
source("/home/jselwyn/Lion/model_functions.R")
source("/home/jselwyn/Lion/demographic parameters.R")
...
initial.females <- seq(1,800,by=1) # vector of number of starting female lionfish
...
for(v in var.rpr){
  ...
  for(S in 1:length(sources)){
    ...
    pdf(paste('/home/jselwyn/Lion/Graphs/Statistical/',names(sources)[S],'-rpr-',v,'-statistical_plots.pdf',sep=''),onefile = T)
    ...
    for(D in 1:length(destinations)){
      ...
      write.csv(statistical.plots[[6]],paste('/home/jselwyn/Lion/graph_data/',names(sources)[S],'-to-',names(destinations)[D],'-rpr-',v,'-plot.data.csv',sep=''))
    }
